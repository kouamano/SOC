...
long long int *lli_calloc_vec(int num){
	long long int *v;
	if((v = calloc(num,(size_t)sizeof(long long int))) == NULL){
		fprintf(stderr,"[E]failed : malloc() at %ld byte.\n",(long int)sizeof(int)*num);
		exit(1);
	}
	return(v);
...
unsigned long long int **lli_calloc_mat(int num, int dim){
	unsigned long long int **m;
	int i;
	if((m = calloc((size_t)num,(size_t)sizeof(unsigned long long int *))) == NULL){
		fprintf(stderr,"[E]failed : calloc() at %ld byte.\n",(long int)sizeof(int *)*num);
		exit(1);
	}
	if((m[0] = calloc((size_t)num*dim,(size_t)sizeof(unsigned long long int))) == NULL){
		fprintf(stderr,"[E]failed : calloc() at %ld byte.\n",(long int)sizeof(int)*num*dim);
		exit(1);
	}
	for(i=1;i<num;i++){
